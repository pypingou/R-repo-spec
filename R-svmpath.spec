%global packname  svmpath
%global rlibdir  %{_libdir}/R/library


Name:             R-%{packname}
Version:          0.95
Release:          1%{?dist}
Summary:          svmpath: the SVM Path algorithm

Group:            Applications/Engineering 
License:          GPL-2
URL:              http://cran.r-project.org/web/packages/%{packname}/index.html
Source0:          http://cran.r-project.org/src/contrib/%{packname}_%{version}.tar.gz
BuildRoot:        %{_tmppath}/%{name}-%{version}-%{release}-root-%(%{__id_u} -n)



BuildRequires:    R-devel tex(latex)  

%description
Computes the entire regularization path for the two-class svm classifier
with essentialy the same cost as a single SVM fit.

%prep
%setup -q -c -n %{packname}

%build

%install
rm -rf %{buildroot}
mkdir -p %{buildroot}%{rlibdir}
%{_bindir}/R CMD INSTALL -l %{buildroot}%{rlibdir} %{packname}
test -d %{packname}/src && (cd %{packname}/src; rm -f *.o *.so)
rm -f %{buildroot}%{rlibdir}/R.css

%clean
rm -rf %{buildroot}

%files
%defattr(-, root, root, -)
%doc %{rlibdir}/svmpath/html
%doc %{rlibdir}/svmpath/DESCRIPTION
%{rlibdir}/svmpath/libs
%{rlibdir}/svmpath/INDEX
%{rlibdir}/svmpath/data
%{rlibdir}/svmpath/Meta
%{rlibdir}/svmpath/R
%{rlibdir}/svmpath/NAMESPACE
%{rlibdir}/svmpath/help

%changelog
* Thu Nov 17 2011 Pierre-Yves Chibon <pingou@pingoured.fr> 0.95-1
- initial package for Fedora