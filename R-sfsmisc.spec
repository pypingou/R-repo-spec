%global packname  sfsmisc
%global rlibdir  %{_datadir}/R/library


Name:             R-%{packname}
Version:          1.0.18
Release:          1%{?dist}
Summary:          Utilities from Seminar fuer Statistik ETH Zurich

Group:            Applications/Engineering 
License:          GPL (>= 2)
URL:              http://cran.r-project.org/web/packages/%{packname}/index.html
Source0:          http://cran.r-project.org/src/contrib/%{packname}_1.0-18.tar.gz
BuildRoot:        %{_tmppath}/%{name}-%{version}-%{release}-root-%(%{__id_u} -n)
BuildArch:        noarch
Requires:         R-core

Requires:         R-stats R-methods R-utils 

BuildRequires:    R-devel tex(latex) R-stats R-methods R-utils 

%description
Useful utilities ['goodies'] from Seminar fuer Statistik ETH Zurich, quite
a few related to graphics; many ported from S-plus times.

%prep
%setup -q -c -n %{packname}

%build

%install
rm -rf %{buildroot}
mkdir -p %{buildroot}%{rlibdir}
%{_bindir}/R CMD INSTALL -l %{buildroot}%{rlibdir} %{packname}
test -d %{packname}/src && (cd %{packname}/src; rm -f *.o *.so)
rm -f %{buildroot}%{rlibdir}/R.css

%clean
rm -rf %{buildroot}

%files
%defattr(-, root, root, -)
%doc %{rlibdir}/sfsmisc/DESCRIPTION
%doc %{rlibdir}/sfsmisc/NEWS
%doc %{rlibdir}/sfsmisc/doc
%doc %{rlibdir}/sfsmisc/html
%{rlibdir}/sfsmisc/R
%{rlibdir}/sfsmisc/demo
%{rlibdir}/sfsmisc/NAMESPACE
%{rlibdir}/sfsmisc/ChangeLog
%{rlibdir}/sfsmisc/data
%{rlibdir}/sfsmisc/INDEX
%{rlibdir}/sfsmisc/Meta
%{rlibdir}/sfsmisc/help

%changelog
* Thu Nov 17 2011 Pierre-Yves Chibon <pingou@pingoured.fr> 1.0.18-1
- initial package for Fedora